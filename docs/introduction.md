# Grasshopperで「好みのデザイン」を探す研究

---

## Grasshopperって何？

Grasshopperは、3DモデリングソフトRhinocerosの上で動くビジュアルプログラミングツールです。
コードを書かなくても、ノードをつなぐだけで**パラメトリックデザイン**が作れます。

![Grasshopperのキャンバス全体像](Figure/Canvas.png)

たとえば「高さ」「回転」「傾き」といったパラメータをスライダーで動かすだけで、形がリアルタイムに変わります。

![スライダーを動かすとモデルが変わる](Figure/slidermodel.gif)

---

## パラメータを変えるだけで、無限の形が生まれる

この研究では、6つのパラメータで定義されるパビリオンを題材にしました。

![Figure 3: 設計パラメータの定義](Figure/Figure%203.png)

| パラメータ | 意味 |
|-----------|------|
| rotation1 / rotation2 | 回転方向 |
| height1 / height2 | 高さ |
| tilt1 / tilt2 | 傾き |

各パラメータは 0〜1 に正規化されており、その組み合わせは無数に存在します。

---

## でも、人間がスライダーを動かすだけだと限界がある

Grasshopperのスライダーは直感的で便利ですが、こんな問題が起きがちです。

- **中間値に偏りがち** — 極端な値（0や1に近い値）を試すのを無意識に避けてしまう
- **高次元で迷子になる** — パラメータが6個以上になると、探索しきれない組み合わせが増える
- **ローカル最適解に陥る** — 最初に気に入った形の近くをうろうろしてしまう

これは認知バイアスの影響です。「なんとなく触りやすい範囲」でデザインが固まってしまいます。

---

## そこで：AIが次の候補を提案してくれる仕組みを作った

この研究では、**ベイズ最適化（Bayesian Optimization）** をGrasshopperに組み込み、
「ユーザーの主観評価」をもとにAIが次に試すべきパラメータを提案する仕組みを構築しました。

![Figure 1: システム全体像](Figure/Figure%201.png)

### 流れはこうです

1. **最初に12案をAIが自動生成**（ソボル列サンプリング）
2. **ユーザーが0〜100点で評価**（美しさ・機能性・新規性などを総合した1スコア）
3. **AIが評価データを学習**し、好みの傾向をモデル化
4. **次に試す3案を提案**（探索と活用のバランスを自動調整）
5. 2〜4を繰り返す

<!-- TODO: Grasshopper上でのUIのスクリーンショット（評価プール・スライダー画面） -->

---

## AIが何を考えているか、画面で見せる

「AIに操られている感」を減らすため、AIの思考プロセスをグラフで可視化しています。

![Figure 2: パラメータ分布プロット](Figure/Figure%202.png)

- 各点 = これまでに評価したデザイン
- 色 = 評価スコア（高いほど明るい）
- 提案の方向性を **「探索寄り」「活用寄り」** とラベルで表示

---

## 実際のUI

左がスライダー操作、右がベイズ法のUIです。

![Figure 4: スライダー法（左）とベイズ法（右）のUI](Figure/Figure%204.png)

ベイズ法では、評価プールから気に入ったデザインを選んで点数をつけるだけ。
次に何を試すかはAIが決めてくれます。

---

## 40人のユーザー実験で検証した結果

参加者40名が両方の方法でデザインを作り、どちらが良かったかを比較しました。

![Figure 6: 主観評価の結果](Figure/Figure%206.png)

| 評価項目 | ベイズ法支持率 | 統計的有意差 |
|---------|------------|-----------|
| 最終デザインの好み | 62.5% | あり |
| 全体的な満足度 | 62.5% | あり |
| デザインの多様性 | **85%** | あり（p<0.001） |
| 収束のしやすさ | 47.5% | なし |
| 使いやすさ | 40% | なし |

### スライダー法との探索範囲の違い

![Figure 5: 最終パラメータ値の比較（ベイズ法 Y軸 vs スライダー法 X軸）](Figure/Figure%205.png)

スライダー法では中間値に集中しているのに対し、ベイズ法では端の値（0や1付近）も積極的に探索されています。

---

## 実際に選ばれたデザインの例

![Figure 7: 参加者による最終デザインの選択例](Figure/Figure%207.png)

ベイズ法を選んだ参加者（C・D）は、スライダー法とは大きく異なる形状のデザインにたどり着いています。
スライダー法で到達できなかった「潜在的な好み」を発見できた例といえます。

---

## まとめ

Grasshopperのスライダーは「知っている範囲」を探るツール。
ベイズ最適化は「まだ知らない好み」を発見するツール。

この研究は、特に**「何が好きかまだよくわからない」段階**のデザイン探索に、
AIを自然に組み込む方法を示しました。

---

> 詳細は [論文ドキュメント](paper.md) を参照してください。
